...
sub cmdlineargs {
	...
	tmsg "Command line args as follows:", 0;

	foreach my $guy (keys(%arghash)) {
		tmsg " $guy = $arghash{$guy}", 0;
	...
	foreach my $override (keys %arghash) {
		if(defined($startup->{$override})) {
			...
			tmsg "Startup: $override => $startup->{$override};", 1;
		} else {
			tmsg "Invalid command line arg $override, bozo", 1;
		}
	}
	return \%arghash;
}

...
#
# Originally penned February-November 2003
# Last update: August 2005
# Dale Anderson
# lime.dale@gmail.com
#
# yay hippies, hippies r nice!
##################################################################

$turtlever = "0.81";

#8/8/05 - new #echo on/off feature (which simply sets $echo, the real feature)
#8/8/05 - defined the host and port up front - $host, and $port
#2/13/04 - added line continuation character \
#2/12/04 - added color says (hardcoded, beginning of lpblock -- '$* = '<c>$*<z>
...
#these two variables will be used by the telnet connection
$host="sneezy.saw.net";
$port=7900;
...
#sub used for debugging and output messages/severity

sub tmsg; #new routine for messages instead of print
          #usage:   tmsg "the message", (optional - volume - -1 low - default: 1)

sub tlog; #a routine that logs tmsgs - not implemented! kluged right now in tmsg!
...
#variable regulating the output of keystroke data
$echo = 0;
...
sub lp {
	...
	############
		...
		{
			unless($linecontinuation eq '') {
				
				$arg = $linecontinuation . $arg;
				$linecontinuation = '';

			}

			if($arg =~ s/\s\\\s*$//) {
				#if that regex matched it automatically tripped the \
				$linecontinuation .= "$arg ";
				last LPBLOCK;
			}
		}
...
sub sp {
	...
	foreach $like (keys %schedules) {
		if($schedules{$like}{seconds} <= $whatever) {
			tmsg "#Time is " . time() . " - time for $like!\n", 2;
			tmsg "#Executing: $schedules{$like}{commands}\n", 2;
...
#	$op = "# %2d $str";
	$op = sprintf "# %2d $str", $loudvol if $ternary_toprint;

	printf $op if $ternary_toprint;
	printf LOGFILE $op if $logging;
...
sub turtle_processor {
	...
	if($cmd eq "#status") { 					#USELESS - STATUS
		...
		#try to do it!
		{
			...
			tmsg "#Set $aliasname to $aliases{$aliasname}.\n", 2;
		}		
		...
		#success mode
		{
			...
			tmsg "#Set \$$varname to " . $variables{$varname} . ".\n", 2
		}
...
#			my $trigexp = shift(@breakout); #second one is regexp
			...
			tmsg "#Set !$trigname for " . $triggers{$trigname}{regexp} . ".\n" , 2;
			tmsg "#Set !$trigname to " . $triggers{$trigname}{trigger} . ".\n" , 2;
		}
...
#		undef(%monitors);
		...
		#the $echo is used when handling keystrokes - whether to print 'em
		# you can still see what you typed with ^R

		if($argmeat =~ m/on/i) {
			tmsg "Echo ENABLED", 2;
			$echo=1;
		} elsif($argmeat =~ m/off/i) {
			tmsg "Echo DISABLED", 2;
			$echo=0;
		} else {
			if ($echo) {
				tmsg "Echo is ON", 3;
